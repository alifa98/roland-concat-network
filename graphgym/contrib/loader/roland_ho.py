"""
Loader for the higher-order trajectory dataset.

For more information: https://zenodo.org/records/8076553

Oct. 24, 2023 edit by Jing
"""
import os
...
def load_single_dataset(dataset_dir: str) -> Graph:
    ...
    columns_name = ['TIMESTAMP', 'LOCATION', 'SRC', 'DST']
    df_trans = pd.read_csv(dataset_dir, sep=',', header=0, names=columns_name)
...
def load_generic_dataset(format, name, dataset_dir):
    if format == 'ho_trajectory': #uci_message
...
register_loader('roland_ho_trajectory', load_generic_dataset)